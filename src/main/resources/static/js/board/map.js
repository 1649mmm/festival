...
let mapOptions = { //지도를 생성할 때 필요한 기본 옵션
    center: new kakao.maps.LatLng(36.1, 126.85), // 지도의 중심좌표
...
function toggleBookmark(boardId) {
    // let bookmarkIconFirst = document.getElementById(`bookmark-${boardId}`);
    // let bookmarkIconSecond = document.getElementById(`bookmark-mark-${boardId}`);
    //
    // // API 호출을 통해 북마크 상태 토글
    // fetch(`/api/map/${boardId}/bookmark`) //  로그인 사용자 ID로 교체
    //     .then(response => {
    //          return response.json()})
    //     .then(data => {
    //         if (data.isBookmarked === true && bookmarkIconFirst.textContent) {
    //             console.log(data.isBookmarked)
    //             alert("yes")
    //             console.log(bookmarkIconFirst.textContent)
    //             // bookmarkIconSecond.textContent = "♥"; // 북마크 추가
    //             bookmarkIconFirst.textContent = "♥"; // 북마크 추가
    //             bookmarkStatus[boardId] = true; // 상태 업데이트
    //
    //         }else if(data.isBookmarked === true && bookmarkIconSecond.textContent) {
    //             bookmarkIconSecond.textContent = "♥"; // 북마크 추가
    //             bookmarkStatus[boardId] = true; // 상태 업데이트
    //
    //         }else if(data.isBookmarked === false && bookmarkIconSecond.textContent) {
    //             bookmarkIconSecond.textContent = "♥"; // 북마크 추가
    //             bookmarkStatus[boardId] = false; // 상태 업데이트
    //
    //         }else if (data.isBookmarked === false && bookmarkIconFirst.textContent) {
    //             alert("no")
    //             console.log(bookmarkIconFirst)
    //             // bookmarkIconSecond.textContent = "♡"; // 북마크 제거
    //             bookmarkIconFirst.textContent = "♡"; // 북마크 제거
    //             bookmarkStatus[boardId] = false; // 상태 업데이트
    //         }
    //     })
    //     .catch(error => {
...
// 초기화 함수 (맵 로드 시 실행)
function initializeMap(festivalList) {
...
viewMapBtn.addEventListener('click', function () {
    map.setCenter(new kakao.maps.LatLng(36.1, 126.85));
    map.setLevel(12);
});
...
viewFestivalBtn.addEventListener('click', function () {
    ...
    if(!(currentCenter.getLat()===36.1 && currentCenter.getLng()===126.85) || currentLevel!==12) {
        map.setCenter(new kakao.maps.LatLng(36.1, 126.85));
        map.setLevel(12);
    }
        ...
        .then(data => {
            ...
            popularFestivals.addEventListener('click', function (event) {
                ...
                if(event.target.tagName === 'LI') {
                    ...
                    if(clickedFestival) { // 축제가 있다면 마커로 이동
                        moveToMarker(clickedFestival);
                    }
                }
            });
        })
...
function addMarker(festival) {
    ...
    let iwContent = `<div class="wrap">
                                <div class="info">
                                    <div class="title">
                                        ...
                                        <button class="bookmark-btn" id="bookmark-mark-btn-${festival.boardId}" aria-label="북마크" onclick="toggleBookmark(${festival.boardId})">
    ...
    kakao.maps.event.addListener(marker, 'click', function() {
        ...
        map.setCenter(new kakao.maps.LatLng(festival.latitude, festival.longitude-0.25));
        map.setLevel(10);
    });
...
function displayFestivalList(page) {
    ...
    pageFestivals.forEach(festival => {
        ...
        // });
            ...
            // $.ajax({
            //     type: "GET",
            //     url: `${location.origin}/api/map/${festival.boardId}/bookmark`, // 동적 URL 생성
            //     success: function (response) {
            //         // 북마크 상태에 따라 버튼 스타일 변경
            //         if (response.isBookmarked) {
            //             $bookmarkIcon.textContent = "♥"; // 북마크된 상태
            //         } else {
            //             $bookmarkIcon.textContent = "♡"; // 북마크 해제된 상태
            //         }
            //         alert(response.message);
            //     },
            //     error: function (error) {
...
function updatePagination() {
    ...
    pagination.innerHTML = `
            <li class="page-item ${startPage === 1 ? 'disabled' : ''}">
                <a class="page-link" href="#" aria-label="Previous" onclick="handlePageChange(${startPage - 1})">
                    <span aria-hidden="true">&laquo;</span>
                </a>
            </li>
            ${pageBtns}
            <li class="page-item ${endPage === totalPages ? 'disabled' : ''}">
                <a class="page-link" href="#" aria-label="Next" onclick="handlePageChange(${endPage + 1})">
