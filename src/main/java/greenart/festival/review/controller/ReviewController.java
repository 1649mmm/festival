...
//            null 검사, "로그인 후 이용해 주세요" 띄우기
    ...
    public ResponseEntity<?> getReviews(@PathVariable Long boardId,
        ...
        List<CommentDTO> reviewList = reviewService.getReviews(boardId);
        List<CommentDTO> reviews = new ArrayList<>();

        for(CommentDTO comment : reviewList) {
    ...
    @GetMapping("/view") //조회
    public ResponseEntity<?> getReplies(@RequestParam Long rootNum,
                                        Model model) {
